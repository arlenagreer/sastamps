...
class LazyLoader {
  constructor(options = {}) {
    this.options = {
      ...
      loadingClass: LAZY_LOADING.CLASSES.LOADING,
      loadedClass: LAZY_LOADING.CLASSES.LOADED,
      errorClass: LAZY_LOADING.CLASSES.ERROR,
      ...options
    };
  ...
  observe(element, loadFunction, options = {}) {
    ...
    const config = {
      ...
      loading: false,
      retries: 0,
      maxRetries: options.maxRetries || LAZY_LOADING.MAX_RETRIES,
  ...
  async loadComponent(element, config) {
    ...
    try {
      ...
      if (config.retries < config.maxRetries) {
        ...
        setTimeout(() => {
          this.loadComponent(element, config);
        }, TIMING.RETRY_DELAY_BASE * config.retries);
      } else {
        // Show error state
            ...
            <div style="text-align: center;">
                <div style="margin-bottom: 0.5rem;">
                    <div class="spinner" style="
                        ...
                        border-radius: 50%;
                        width: ${LAZY_LOADING.SPINNER_SIZE}px;
                        height: ${LAZY_LOADING.SPINNER_SIZE}px;
  ...
  static getCalendarPlaceholder() {
    return `
            <div class="calendar-placeholder" style="
                ...
                padding: 2rem;
                text-align: center;
                min-height: ${LAZY_LOADING.CALENDAR_MIN_HEIGHT || 300}px;
                display: flex;
